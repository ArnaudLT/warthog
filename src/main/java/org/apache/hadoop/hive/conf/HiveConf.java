...
public class HiveConf extends Configuration {
    ...
    public String getLogIdVar(String defaultValue) {
        ...
        if (retval.length() > LOG_PREFIX_LENGTH) {
            LOG.warn("The original log id prefix is " + retval + " has been truncated to "
    ...
    private void initialize(Class<?> cls) {
        ...
        if (getBoolVar(HiveConf.ConfVars.HIVECONFVALIDATION)) {
            ...
            for (Map.Entry<String, String> entry : this) {
                ...
                if (var == null) {
                    LOG.warn("HiveConf of name " + key + " does not exist");
                } else if (!var.isType(entry.getValue())) {
                    LOG.warn("HiveConf " + var.varname + " expects " + var.typeString() + " type value");
                }
            }
